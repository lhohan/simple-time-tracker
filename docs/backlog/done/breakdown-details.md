...
### Task Aggregation by First Tag

Tasks are aggregated by their **first tag** (the primary project/context tag). For example:
- Entry: `- #work #project-a 2h Implement feature X` → aggregated under `project-a`
- Entry: `- #work #project-b 1h Code review` → aggregated under `project-b`

This provides a higher-level view of what was worked on, grouped by project rather than by individual task descriptions.
...
Entries without tags (only description) show `<no description>` placeholder (consistent with existing `--details` behavior).
...
**Day breakdown with details (aggregated by first tag):**
```text
...
2020-01..3h 00m
  2020-W01..1h 00m
    - project-a..1h 00m (100%)
  2020-W02..2h 00m
    - project-b..2h 00m (100%)
```
...
- **Red:** Write test expecting tasks (aggregated by first tag) under day header
- **Green:** 
  - Extend `BreakdownGroup` with `tasks: Vec<TaskSummary>` field
  - Update `aggregate_tasks_from_entries()` to use first tag instead of description
...
- **Red:** Write test with entries on different days, different first tags
- **Green:** Should pass if Test 2 correct; fix if not
- **Refactor:** Clean up duplication

### Test 4: Task Aggregation by First Tag
**Test:** `breakdown_day_with_details_should_aggregate_by_first_tag`
- **Red:** Write test with multiple entries having same first tag
- **Green:** Verify aggregation sums minutes for same first tag
